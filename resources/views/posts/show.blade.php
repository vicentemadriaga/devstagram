...
<div class="container mx-auto md:flex">
    <div class=" md:w-1/2 items-center">
        <a><img src="{{ asset('uploads') . '/' . $post->imagen}}" alt="imagen del post {{ $post->titulo}}" height="500px" width="500px"></a>
        ...
        <div>
            ...
            <p class=" font-bold">Autor: {{ $post->user->username}}</p>
        ...
        <form action="{{ route('post.destroy', $post)}}" method="POST">
             @method('DELETE')
             @csrf
            <input type="submit" value="Eiminar publicacion" class="bg-red-500 hover:bg-red-600 rounded text-white mt-4 cursor-pointer p-2"  >
        </form>
        @endif
        @endauth
    </div>
    <div class=" md:w-1/2 p-5">
        ...
        <div class="shadow bg-white p-5 mb-5">
            ...
            @guest
            <p class=" text-xl font-bold text-center mb-4">Inicia sesion para ingresar un comentario</p>
            <div class="items-center">
                <a class=" flex items-center gap-2 bg-white border p-2 text-gray-600 rounded text-sm uppercase font-bold  cursor-pointer" href="{{ route('login') }}">
